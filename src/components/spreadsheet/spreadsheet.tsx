...
import { CsvUtils } from './csv-utils';  // Adjust path if needed
...
export class AppSpreadsheet {
  @State() sheet: Sheet = this.createInitialSheet(1000, 26);
...
this.lastChangedValue = value;
  ...
  console.log(
      `📌 cellSelected → now selected (${row},${col}) value="${cell.value}", formula="${cell.formula}"`
    );
  }
...
handleKeyDown(e: KeyboardEvent) {
  ...
  switch (e.key) {
    case 'ArrowUp':    this.selectCell(Math.max(0, row - 1), col); break;
    case 'ArrowDown':  this.selectCell(Math.min(this.sheet.length - 1, row + 1), col); break;
    case 'ArrowLeft':  this.selectCell(row, Math.max(0, col - 1)); break;
    case 'ArrowRight': this.selectCell(row, Math.min(this.sheet[0].length - 1, col + 1)); break;
  }
...
//   private cloneSheet(sheet: Sheet, method: "spread" | "json" = "spread"): Sheet {
//   if (method === "json") {
//     return JSON.parse(JSON.stringify(sheet));
//   }
//   return sheet.map(row => row.map(cell => ({ ...cell })));
// }
...
//  console.log('⏪ restoreSnapshot: BEFORE restore, current A1 =', this.sheet[0]?.[0]?.value ?? '');
//     console.log('⏪ restoreSnapshot: SNAP A1 =', snap.sheet[0]?.[0]?.value ?? '');
  ...
  // this.sheet = this.sheet.map(row => [...row]);
  //  if (shouldRecalc) this.recalculateFormulas();

  //  this.sheet = snap.sheet.map(row => row.map(cell => ({ ...cell })));



   this.sheet = [...this.sheet];  

  // 🔹 Update formulaDraft so formula bar shows correct value
...
// setTimeout(() => {
//     this.recalculateFormulas();
//     this.sheet = this.sheet.map(row => [...row]); // Refresh UI
//   }, 0);
...
//   // Once we make a new change, redo stack is invalid
//   this.future = [];
//   console.log(
//       `📝 pushHistory → history.length=${this.history.length}, future cleared, last A1=`,
//       snap.sheet[0]?.[0]?.value ?? '',
//     );
...
//   //  console.log(
//   //     '🔁 undo: pushed CURRENT to future, future.length=',
//   //     this.future.length,
//   //     'current A1=',
//   //    this.takeSnapshot() .sheet[0]?.[0]?.value ?? '',
//   //   );

//   // Restore last state from history
 
//  console.log('➡️ Moved CURRENT to future (redo stack)');

//  const prev = this.history.pop();
// //  console.log('🔍 undo: popping PREVIOUS from history, history.length after pop=', this.history.length);
    
//   if (prev) {
//      console.log('⬅️ Restoring PREVIOUS snapshot');
//     this.restoreSnapshot(prev);  // restores sheet
//   }
...
//   //  console.log(
//   //     '🔁 redo: pushed CURRENT back to history, history.length=',
//   //     this.history.length,
//   //     'current A1=',
//   //     this.takeSnapshot().sheet[0]?.[0]?.value ?? '',);
...
//     if (next) {
//       console.log('➡️ Restoring NEXT snapshot');
...
//   this.updateCellValue(this.selectedRow, this.selectedCol, this.clipboardValue);
//   console.log("📌 Pasted:", this.clipboardValue);
// }
...
//   if (this.sheet[row][col].value === value) return; // 🚨 Prevent useless history
  
//   this.pushHistory(); // ✔ BEFORE mutation
...
//   const active = this.sheet[this.selectedRow][this.selectedCol];
//   this.formulaDraft = active.formula ?? active.value ?? '';

//    console.log('📐 After updateCellValue → A1 =', this.sheet[0]?.[0]?.value ?? '');
// }

// updateCellValue(row: number, col: number, value: string) {
//   if (this.sheet[row][col].value === value) return;

//   // Save snapshot including selection
//   this.pushHistory();

//   const sheetCopy = this.cloneSheet(this.sheet);
//   sheetCopy[row][col].value = value;
//   sheetCopy[row][col].formula = value.startsWith('=') ? value : undefined;
//   this.sheet = sheetCopy;

//   // Set selection to the edited cell
//   this.selectedRow = row;
//   this.selectedCol = col;
...
updateCellValue(row: number, col: number, value: string) {
  ...
  // this.pushHistory();
   ...
   console.log(
    ...
    this.history[this.history.length - 1]?.sheet[row][col],
    '(Expected old value)',
    '   New incoming value =', value
  );
  ...
  // 4️⃣ Recalculate ONLY formula cells
  // this.recalculateFormulas();
  ...
  getCellFormulaOrValue(): string {
    // const cell = this.sheet[this.selectedRow][this.selectedCol];
    // return cell.formula ?? cell.value; // show formula if exists
  ...
  //   }
  //   this.sheet = [...this.sheet]; // Re-render
  // }

  onFormulaInput(value: string) {
    // const sheetCopy = this.cloneSheet(this.sheet);
    // const cell = sheetCopy[this.selectedRow][this.selectedCol];

    // cell.formula = value.startsWith('=') ? value : undefined;
    // if (!value.startsWith('=')) cell.value = value;

    // this.sheet = sheetCopy; // Only update needed
  ...
  // applyFormula() {
  //   const sheetCopy = [...this.sheet];
  //   console.log("📍 BEFORE FORMULA", this.sheet[0][0].value, this.sheet[1][0].value);

  //   const cell = { ...sheetCopy[this.selectedRow][this.selectedCol] };
  // console.log('Formula Entered:', cell.value);
  //   if (cell.value.startsWith('=')) {
  //     cell.formula = cell.value;
  //     console.log('Formula Raw:', cell.formula);

  //     const engine = new FormulaEngine(sheetCopy);
  //     const result = engine.evaluate(cell.formula);
  //     cell.value = result?.toString() ?? '#ERROR';
  //   }

  //   sheetCopy[this.selectedRow][this.selectedCol] = cell;
  //   this.sheet = sheetCopy;
  //   console.log('Engine sheet snapshot:', this.sheet);
  //   this.recalculateFormulas();
  // }
  ...
  //   console.log('📍 BEFORE FORMULA', sheetCopy[0][0].value, sheetCopy[1][0].value);
  ...
  //   this.sheet = sheetCopy; // update state once
  //   console.log('Engine sheet snapshot:', this.sheet);
 ...
 console.log('📥 Formula Snapshot BEFORE update', this.history[this.history.length - 1]?.sheet[row][col]);
  ...
  if (value.startsWith('=')) {
    cell.formula = value;
    // const engine = new FormulaEngine();
    // const result = engine.evaluate(value, sheetCopy);
    // cell.value = result?.toString() ?? '#ERROR';
  } else {
    cell.value = value;
    cell.formula = undefined;
  ...
  this.sheet = sheetCopy;
  this.recalculateFormulas();
    // const active = this.sheet[row][col];
    // this.formulaDraft = active.formula ?? active.value ?? '';
  ...
  //           <span>{this.getCellAddress()}</span> {/* Example: E5 */}
  //           <input
  ...
  //             <div class="corner-cell"></div> {/* Empty top-left */}
  ...
  //           {/* 🔹 Rows with Row Index + Cells */}
  //           {this.sheet.map((row, rIndex) => (
  //             <div class="row">
...
render() {
  ...
  const bottomSpacerHeight =
    (this.sheet.length - endRow - 1) * baseRowHeight;
      ...
      <div class="header">
        <div class="toolbar">
          ...
          <div class="color-picker">
            <button
              class="icon-btn"
              onClick={() => this.textColorInput?.click()}
            >
              <i class="icon-text-color"></i>
            </button>
            <input
          ...
          <div class="color-picker">
            <button
              class="icon-btn"
              onClick={() => this.bgColorInput?.click()}
            >
              <i class="icon-fill-color"></i>
            </button>
            <input
          ...
          {/* <button onClick={() => this.importInput?.click()}>Import CSV</button>
<button onClick={() => this.exportCSV()}>Export CSV</button> */}

<button class="csv-btn" onClick={() => this.importInput?.click()}>📥 Import</button>
<button class="csv-btn" onClick={() => this.exportCSV()}>📤 Export</button>


<input
  type="file"
  accept=".csv,text/csv"
  ref={el => (this.importInput = el as HTMLInputElement)}
  style={{ display: 'none' }}
...
/>
        ...
        <div class="formula-bar">
          ...
          <input
            value={this.getCellFormulaOrValue()}
            onInput={e =>
              this.onFormulaInput((e.target as HTMLInputElement).value)
            }
            onKeyDown={e => e.key === 'Enter' && this.applyFormula()}
          />
        </div>
        ...
        <div class="sheet">
          ...
          <div class="row header-row">
            ...
            {this.sheet[0].map((_, colIndex) => (
              <div
                class="column-header"
                style={{ width: `${this.columnWidths[colIndex]}px` }}
              >
                {this.getColumnName(colIndex)}
                <div
                  class="resize-handle-column "
                  onMouseDown={e => this.startColumnResize(e, colIndex)}
                ></div>
              </div>
            ))}
          </div>
          ...
          {this.sheet.slice(startRow, endRow + 1).map((row, idx) => {
            ...
            return (
              <div class="row">
                <div
                  class="row-header"
                  style={{ height: `${this.rowHeights[rIndex]}px` }}
                >
                  {rIndex + 1}
                  <div
                    class="resize-handle-row"
                    onMouseDown={e => this.startRowResize(e, rIndex)}
                  ></div>
                </div>
                ...
                {row.map((cell, cIndex) => (
                  <app-cell
                    ...
                    col={cIndex}
                    isSelected={
                      this.selectedRow === rIndex &&
                      this.selectedCol === cIndex
                    }
                    isInRange={
    rIndex >= this.selectionRange.r1 &&
    rIndex <= this.selectionRange.r2 &&
    cIndex >= this.selectionRange.c1 &&
    cIndex <= this.selectionRange.c2
  }
                    ...
                    // selected={this.selectedRow === rIndex && this.selectedCol === cIndex ? 'true' : 'false'}
                    style={{
                      '--cell-width': `${this.columnWidths[cIndex]}px`,
                      '--cell-height': `${this.rowHeights[rIndex]}px`,
                    }}
